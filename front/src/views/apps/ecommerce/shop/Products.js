...
const ProductsPage = props => {
  ...
  const renderPageItems = () => {
    const arrLength =
      store.totalProducts !== 0 && store.results.length !== 0 ? Number(store.totalProducts) / store.results.length : 3
  ...
  const handleNext = () => {
    if (store.params.page !== Number(store.totalProducts) / store.results.length) {
          ...
          <Fragment>
            ...
            <Pagination className='d-flex justify-content-center ecommerce-shop-pagination mt-2'>
              ...
              <PaginationItem
                className='next-item'
                onClick={() => handleNext()}
                disabled={store.params.page === Number(store.totalProducts) / store.results.length}
              >
                <PaginationLink href='/' onClick={e => e.preventDefault()}></PaginationLink>
              </PaginationItem>
            </Pagination>
